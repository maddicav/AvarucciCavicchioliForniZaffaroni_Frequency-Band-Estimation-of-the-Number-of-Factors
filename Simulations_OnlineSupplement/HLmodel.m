...
% described in Section S.2.1 in Avarucci et al 2025, Online Supplement, "Simulation Design: First Experiment (HL)"
%
% INPUTS    model  :  ('AR' or 'MA')
%           N,T    :  panel dimension
%          q      :  number of factors/shocks
% OUTPUTS   panel  : (T x N) panel data (common component + idiosyncratic component)
%           common : (T x N) common component
...
coeff = (1+0.5*rand((i2+1)*(N+4),N)).*C2;
idi = e2*coeff;   
idi = (idi- ones(T,1)*mean(idi))./(ones(T,1)*std(idi));
end
%-------------------------------------------------------------------------
